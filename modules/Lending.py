...
def init(cfg, api1, log1, data, maxtolend, dry_run1, analysis, notify_conf1):
    ...
    min_daily_rate = Decimal(Config.get("BOT", "mindailyrate", None, 0.0003, exchangeMax)) / 100
    max_daily_rate = Decimal(Config.get("BOT", "maxdailyrate", None, 0.0003, exchangeMax)) / 100
    ...
    xday_threshold = float(Config.get("BOT", "xdaythreshold", None, 0.0003, 5)) / 100
...
def get_frr_or_min_daily_rate(cur):
    ...
    if cur in coin_cfg:
        ...
        min_daily_rate = Decimal(Config.get("BOT", "mindailyrate", None, 0.0003, 5)) / 100
        frrasmin = Config.getboolean('BOT', 'frrasmin', False)
        frrdelta = Decimal(Config.get('BOT', 'frrdelta', 0.0000))
